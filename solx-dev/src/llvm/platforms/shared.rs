...
/// afterwards. `lld-*` and `mlir-*` are included because
/// `shared_build_opts_projects` enables both projects.
///
/// When `enable_utils` is true, `FileCheck` is added to the whitelist so it
/// lands in the install prefix's `bin/`. LLVM's `add_llvm_utility` registers each utility
...
/// configure time.
///
/// `llvm-lit` is **not** listed here: upstream LLVM has no install rule for `llvm-lit` (it's
