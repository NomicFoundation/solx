...
// CHECK: sol.func @{{.*addressDefault.*}}() -> !sol.address
...
// CHECK: sol.func @{{.*dynArrayDefault.*}}() -> !sol.array<? x ui256, Memory>
...
// CHECK: sol.func @{{.*fixedArrayDefault.*}}() -> !sol.array<2 x ui256, Memory>
...
contract C {
    ...
    function addressDefault() public pure returns (address) {}
    ...
    function dynArrayDefault() public pure returns (uint256[] memory) {}

    function enumDefault() public pure returns (E) {}

    function fixedArrayDefault() public pure returns (uint256[2] memory) {}
