...
// CHECK: sol.func @{{.*int_narrow.*}}
// CHECK:   sol.cast %{{.*}} : si256 to si16

// CHECK: sol.func @{{.*int_to_uint.*}}
// CHECK:   sol.cast %{{.*}} : si256 to ui256

// CHECK: sol.func @{{.*int_widen.*}}
// CHECK:   sol.cast %{{.*}} : si8 to si256

// CHECK: sol.func @{{.*sign_to_signed.*}}
// CHECK:   sol.cast %{{.*}} : ui8 to si8

// CHECK: sol.func @{{.*sign_to_unsigned.*}}
// CHECK:   sol.cast %{{.*}} : si8 to ui8
