...
// solc emits the generic !sol.func_ref<() -> ()> and leans on the store slot. Both backends emit the
// functions in source declaration order, so each backend's body is checked under its own prefix.
