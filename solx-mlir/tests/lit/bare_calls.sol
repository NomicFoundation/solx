...
// drop the value operand. Each yields `(i1 status, bytes ret_data)`. The input
// must be a memory buffer, so a storage or calldata argument is copied to memory
// with `sol.data_loc_cast` before the call.
