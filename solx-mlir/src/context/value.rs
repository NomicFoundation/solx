...
impl<'context, 'block> Value<'context, 'block> {
    ...
    /// Compares this value with `other` under `predicate`, reconciling their types
    /// first when they differ — the comparison-operand coercion `==` / `<` apply
    /// before [`Self::compare`]'s raw `sol.cmp`.
    ///
    /// Equal types compare directly. Two fixed-bytes operands of different widths
    /// (`bytes3("abc") == bytes4("abc")`) widen the narrower to the larger width AS
    /// fixed-bytes: `bytesN` are LEFT-aligned, so the narrower zero-extends on the
    /// right and the two share a word — bridging each through its own width integer
    /// would right-align them (`ui24` 0x616263 vs `ui32` 0x61626300) and compare
    /// wrong. Mixed integers widen each to 256 bits preserving ITS OWN signedness
    /// (a signed operand sign-extends, so a negative is not reinterpreted as a huge
    /// unsigned), then retype to a common signedness — signed if either is, so
    /// `(-10) < 10` stays a signed comparison rather than an unsigned one.
    pub fn compare_coerced(
        ...
        if self.r#type() == other.r#type() {
            return self.compare(other, predicate, builder, block);
        }
        if let (Some(lhs_width), Some(rhs_width)) = (
            self.r#type().fixed_bytes_or_byte_width(),
            other.r#type().fixed_bytes_or_byte_width(),
        ) {
            let common_width = lhs_width.max(rhs_width);
            let common = Type::fixed_bytes(builder.context, common_width);
            let lhs_common = if lhs_width == common_width {
                self
            } else {
                self.cast(common, builder, block)
            };
            let rhs_common = if rhs_width == common_width {
                other
            } else {
                other.cast(common, builder, block)
            };
            return lhs_common.compare(rhs_common, predicate, builder, block);
        }
        let signed_lhs = IntegerType::try_from(self.r#type().into_mlir())
            .is_ok_and(|integer| integer.is_signed());
        let signed_rhs = IntegerType::try_from(other.r#type().into_mlir())
            .is_ok_and(|integer| integer.is_signed());
        let signed_256 = Type::new(IntegerType::signed(builder.context, 256).into());
        let unsigned_256 = Type::unsigned(builder.context, solx_utils::BIT_LENGTH_FIELD);
        let lhs_wide = self.cast(
            if signed_lhs { signed_256 } else { unsigned_256 },
            builder,
            block,
        );
        let rhs_wide = other.cast(
            if signed_rhs { signed_256 } else { unsigned_256 },
            builder,
            block,
        );
        ...
        let lhs_common = if lhs_wide.r#type() == common {
            ...
            lhs_wide.cast(common, builder, block)
        };
        let rhs_common = if rhs_wide.r#type() == common {
            rhs_wide
        } else {
            rhs_wide.cast(common, builder, block)
        };
        lhs_common.compare(rhs_common, predicate, builder, block)
    }
