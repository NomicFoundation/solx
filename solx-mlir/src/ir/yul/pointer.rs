//!
//! A `!yul.ptr`: the address a Yul variable's word is held at.
//!
...
/// parameter, a function's return variable - is held behind a pointer rather than in an SSA value,
/// because Yul assigns to a name after binding it. A Solidity variable an assembly block reaches
/// also surfaces as one, through the `sol.yul_*` bridge ops on [`Place`](crate::Place).
#[derive(Clone, Copy)]
...
impl<'context, V> From<V> for Pointer<'context>
where
    V: ValueLike<'context>,
{
...
impl<'context> IntoOds<MlirValue<'context, 'context>> for Pointer<'context> {
