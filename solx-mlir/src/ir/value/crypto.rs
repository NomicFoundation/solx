...
impl<'context> Value<'context> {
    ...
    pub fn keccak256(data: Self, context: &Context<'context>) -> Self {
    ...
    pub fn ecrecover(hash: Self, v: Self, r: Self, s: Self, context: &Context<'context>) -> Self {
    ...
    pub fn sha256(data: Self, context: &Context<'context>) -> Self {
        let digest = Type::fixed_bytes(context.melior, 32).into_mlir();
        Self::from(mlir_op!(context, Sha256Operation.data(data).result(digest)))
    }

    /// Emits `sol.ripemd160` (precompile `0x03`) over `data`, yielding a `bytes20` digest.
    pub fn ripemd160(data: Self, context: &Context<'context>) -> Self {
