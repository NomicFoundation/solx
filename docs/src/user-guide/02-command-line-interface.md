...
With the default *solc* front-end, **solx** only passes remappings through to *solc*. With the Slang front-end, **solx** applies them itself, following *solc*'s semantics.
...
These options are used to specify Solidity import resolution settings. They are not used by **solx** and only passed through to *solc*. The Slang front-end ignores them and resolves imports only against the sources it is given.
