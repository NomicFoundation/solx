...
impl<'state, 'context, 'block> ExpressionEmitter<'state, 'context, 'block> {
    ...
    pub(crate) fn emit_store(
        ...
        block.append_operation(
            OperationBuilder::new(solx_mlir::ops::sol::STORE, location)
    ...
    pub(crate) fn emit_alloca(
        ...
        // sol.alloca returns a sol.ptr<allocType, Stack> pointer type.
        let ptr_type = melior::ir::Type::parse(context, "!sol.ptr<i256, Stack>")
            .expect("valid sol.ptr type syntax");

        block
            .append_operation(
                OperationBuilder::new(solx_mlir::ops::sol::ALLOCA, location)
                    .add_attributes(&[(
                        Identifier::new(context, "alloc_type"),
    ...
    fn emit_load(
        ...
        Ok(block
            .append_operation(
                OperationBuilder::new(solx_mlir::ops::sol::LOAD, location)
    ...
    fn emit_address_call(
        ...
        let gas = self.emit_i256_constant(Self::TRANSFER_GAS_STIPEND, &block);
        let zero = self.emit_i256_constant(0, &block);
        let null_pointer = self.emit_inttoptr(zero, self.state.pointer(AddressSpace::Heap), &block);
        ...
        if revert_on_fail {
            ...
            let revert_pointer = self.emit_inttoptr(revert_zero, heap_pointer_type, &revert_ref);
    ...
    fn emit_inttoptr(
        &self,
        value: Value<'context, 'block>,
        pointer_type: melior::ir::Type<'context>,
        block: &BlockRef<'context, 'block>,
    ) -> Value<'context, 'block> {
        self.state.emit_inttoptr(value, pointer_type, block)
    }
    ...
    fn emit_intrinsic_call(
        ...
        let operation = block.append_operation(
            OperationBuilder::new(name, self.state.location())
