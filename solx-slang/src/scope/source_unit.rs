...
pub struct SourceUnitScope<'context> {
    ...
    function_signatures: HashMap<NodeId, Function<'context>>,
}
...
impl<'context> SourceUnitScope<'context> {
    ...
    pub fn contract<R>(
        ...
        object: Object,
        emit: impl FnOnce(&mut ContractScope<'_, 'context>) -> R,
    ) -> R {
        self.mlir.current_contract_type = Some(contract_type);
        let result = emit(&mut ContractScope::new(self, contract, object));
