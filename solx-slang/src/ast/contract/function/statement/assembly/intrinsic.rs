...
use solx_mlir::ods::yul::AddModOperation as YulAddModOp;
use solx_mlir::ods::yul::AddressOperation as YulAddressOp;
use solx_mlir::ods::yul::BalanceOperation as YulBalanceOp;
use solx_mlir::ods::yul::ByteOperation as YulByteOp;
use solx_mlir::ods::yul::CallDataCopyOperation as YulCallDataCopyOp;
use solx_mlir::ods::yul::CallDataLoadOperation as YulCallDataLoadOp;
use solx_mlir::ods::yul::CallDataSizeOperation as YulCallDataSizeOp;
use solx_mlir::ods::yul::CallOperation as YulCallOp;
use solx_mlir::ods::yul::CodeCopyOperation as YulCodeCopyOp;
use solx_mlir::ods::yul::CodeSizeOperation as YulCodeSizeOp;
use solx_mlir::ods::yul::Create2Operation as YulCreate2Op;
use solx_mlir::ods::yul::CreateOperation as YulCreateOp;
use solx_mlir::ods::yul::DelegateCallOperation as YulDelegateCallOp;
use solx_mlir::ods::yul::DivOperation as YulDivOp;
use solx_mlir::ods::yul::ExtCodeHashOperation as YulExtCodeHashOp;
use solx_mlir::ods::yul::ExtCodeSizeOperation as YulExtCodeSizeOp;
use solx_mlir::ods::yul::InvalidOperation as YulInvalidOp;
use solx_mlir::ods::yul::Keccak256Operation as YulKeccak256Op;
use solx_mlir::ods::yul::LogOperation as YulLogOp;
use solx_mlir::ods::yul::MCopyOperation as YulMCopyOp;
use solx_mlir::ods::yul::MLoadOperation as YulMLoadOp;
use solx_mlir::ods::yul::MStore8Operation as YulMStore8Op;
use solx_mlir::ods::yul::MStoreOperation as YulMStoreOp;
use solx_mlir::ods::yul::ModOperation as YulModOp;
use solx_mlir::ods::yul::MulModOperation as YulMulModOp;
use solx_mlir::ods::yul::NotOperation as YulNotOp;
...
use solx_mlir::ods::yul::ReturnOperation as YulReturnOp;
use solx_mlir::ods::yul::RevertOperation as YulRevertOp;
use solx_mlir::ods::yul::SDivOperation as YulSDivOp;
use solx_mlir::ods::yul::SLoadOperation as YulSLoadOp;
use solx_mlir::ods::yul::SModOperation as YulSModOp;
use solx_mlir::ods::yul::SStoreOperation as YulSStoreOp;
use solx_mlir::ods::yul::SarOperation as YulSarOp;
use solx_mlir::ods::yul::SelfBalanceOperation as YulSelfBalanceOp;
use solx_mlir::ods::yul::SignExtendOperation as YulSignExtendOp;
use solx_mlir::ods::yul::StaticCallOperation as YulStaticCallOp;
use solx_mlir::ods::yul::StopOperation as YulStopOp;
use solx_mlir::ods::yul::TLoadOperation as YulTLoadOp;
use solx_mlir::ods::yul::TStoreOperation as YulTStoreOp;

impl<'state, 'context, 'block> StatementEmitter<'state, 'context, 'block> {
    /// Lowers a Yul intrinsic (built-in EVM opcode) `name(arguments)` to its
    /// corresponding `sol.*` / `yul.*` operation, returning the resulting
    /// 256-bit word and the continuation block.
    ...
    pub fn emit_yul_intrinsic(
        ...
        match name {
            ...
            "lt" => {
                let cmp =
                    builder.emit_sol_cmp(arguments[0], arguments[1], CmpPredicate::Lt, &block);
                let value = builder.emit_sol_cast(cmp, ui256, &block);
                Ok((value, block))
            }
            "gt" => {
                let cmp =
                    builder.emit_sol_cmp(arguments[0], arguments[1], CmpPredicate::Gt, &block);
                let value = builder.emit_sol_cast(cmp, ui256, &block);
                Ok((value, block))
            }
            "eq" => {
                let cmp =
                    builder.emit_sol_cmp(arguments[0], arguments[1], CmpPredicate::Eq, &block);
                let value = builder.emit_sol_cast(cmp, ui256, &block);
                Ok((value, block))
            }
            "slt" => {
            ...
            "selfbalance" => {
                let value = block
                    .append_operation(
                        YulSelfBalanceOp::builder(ctx, loc)
                            .out(i256_signless)
                            .build()
                            .into(),
                    )
            ...
            "calldatasize" => {
                let value = block
                    .append_operation(
                        YulCallDataSizeOp::builder(ctx, loc)
                            .out(i256_signless)
                            .build()
                            .into(),
                    )
