...
//!
//! Walks a Solidity `assembly { ... }` block and lowers its Yul body to MLIR:
//! straight-line arithmetic, comparison and bitwise ops, memory / storage
//! load and store, and the EVM context / opcode intrinsics; Yul control flow
//! (`if` / `for` / `switch`); single- and multi-value `let` bindings; and
//! user-defined Yul functions, which are inlined at every call site
//! (recursion is rejected, as it would otherwise inline forever).
//!
...
impl<'state, 'context, 'block> StatementEmitter<'state, 'context, 'block> {
    ...
    fn emit_yul_statement(
        ...
        match statement {
            YulStatement::YulVariableAssignmentStatement(assignment) => {
                ...
                if variables.len() != 1 {
                    ...
                    if !self.yul_functions.contains_key(&callee) {
                        unimplemented!(
                            "multi-value yul assignment RHS is not a user-defined function"
                        );
                    }
                    ...
                    for (path, value) in variables.iter().zip(values) {
                        let name = path.iter().next().expect("empty yul lvalue path").name();
                ...
                let initials: Vec<Option<Value<'context, 'block>>> =
                    if let Some(value_node) = declaration.value() {
                        ...
                        if variable_count > 1 {
                            ...
                            let name = path.iter().next().expect("empty yul callee path").name();
            ...
            YulStatement::YulForStatement(for_statement) => {
                ...
                let cond_bool = builder.emit_sol_cmp(cond_u, zero, CmpPredicate::Ne, &cond_end);
            ...
            YulStatement::YulBlock(yul_block) => {
                ...
                let saved_keys: Vec<String> = self.yul_functions.keys().cloned().collect();
    ...
    fn emit_yul_switch_chain(
        ...
        for case in value_cases {
            ...
            let (literal_value, new_block) = self.emit_yul_expression(&synthetic, current)?;
            current = new_block;
            let builder = &self.state.builder;
            let condition =
    ...
    fn emit_yul_expression(
        ...
        match expression {
            ...
            YulExpression::YulFunctionCallExpression(call) => {
                ...
                let name = path.iter().next().expect("empty yul function path").name();
    ...
    fn yul_literal_to_word(literal: &YulLiteral) -> anyhow::Result<BigInt> {
        Ok(match literal {
            ...
            YulLiteral::DecimalLiteral(decimal) => {
                BigInt::from_str_radix(decimal.unparse().trim(), 10)
        ...
        if path.len() == 2 {
                    ...
                    parts[0].resolve_to_definition()
            {
                ...
                let value = if member == "slot" {
                    let slot_big = BigInt::from_bytes_be(
                        num_bigint::Sign::Plus,
                        &storage_slot.slot.to_be_bytes_vec(),
                    );
                    builder.emit_constant(&slot_big, ui256, &block)
                } else {
                    builder.emit_sol_constant(i64::from(storage_slot.byte_offset), ui256, &block)
                };
        ...
        *depth += 1;
        ...
        for return_name in return_names.iter() {
            ...
            let cast = if loaded.r#type() == ui256 {
                ...
                self.state.builder.emit_sol_cast(loaded, ui256, &body_block)
            };
