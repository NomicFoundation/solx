...
pub struct ExpressionEmitter<'state, 'context, 'block> {
    ...
    pub region: &'state Region<'context>,
    ...
    pub fn emit(
        ...
        match expression {
            ...
            Expression::ExponentiationExpression(expression) => {
                ...
                let result =
                    self.emit_llvm_operation(solx_mlir::Builder::SOL_EXP, lhs, rhs, &block)?;
                Ok((result, block))
            }
            Expression::ConditionalExpression(conditional) => {
                ...
                then_end
                    .append_operation(self.state.builder().llvm_br(&merge_block, &[then_value]));

                let false_expression = conditional.false_expression();
                let (else_value, else_end) = self.emit(&false_expression, else_block)?;
                else_end
                    .append_operation(self.state.builder().llvm_br(&merge_block, &[else_value]));
