...
pub struct ExpressionEmitter<'state, 'context> {
    /// Variable environment.
    pub environment: &'state Environment<'context>,
...
impl<'state, 'context> ExpressionEmitter<'state, 'context> {
    ...
    pub fn new(
        environment: &'state Environment<'context>,
    ...
    pub fn emit_value(
        ...
        self.emit(expression, context)?
            .ok_or_else(|| anyhow::anyhow!("expression produced no value"))
    }

    /// Emits MLIR for an expression, appending operations at the insertion cursor.
    ///
        ...
        match expression {
            Expression::DecimalNumberExpression(decimal_number) => {
                ...
                let result_type = self
                    .resolve_slang_type(decimal_number.get_type(), context)
                    .expect("binder types every decimal literal node");
                let constant = Value::constant_from_bigint(&value, result_type, context);
                Ok(Some(constant))
            }
            Expression::HexNumberExpression(hex_number) => {
                ...
                let result_type = self
                    .resolve_slang_type(hex_number.get_type(), context)
                    .expect("binder types every hex literal node");
                let constant = Value::constant_from_bigint(&value, result_type, context);
                Ok(Some(constant))
            }
            Expression::TrueKeyword(_) => {
                let constant = Value::boolean(true, context);
                Ok(Some(constant))
            }
            Expression::FalseKeyword(_) => {
                let constant = Value::boolean(false, context);
                Ok(Some(constant))
            }
            Expression::ThisKeyword(_) => {
                ...
                let value = Value::this(contract_type, context);
                Ok(Some(value))
            }
            Expression::StringExpression(string_expression) => {
                ...
                let value = Value::string_literal(text, context);
                Ok(Some(value))
            }
            Expression::Identifier(identifier) => {
                ...
                match identifier.resolve_to_definition() {
                    ...
                    Some(Definition::Variable(_) | Definition::Parameter(_)) => {
                        ...
                        let value = pointer.load(element_type, context);
                        Ok(Some(value))
                    }
            ...
            Expression::AssignmentExpression(assign) => {
                self.emit_assignment(assign, context).map(Some)
            }
            Expression::AdditiveExpression(expression) => {
                let result_type = self.resolve_slang_type(expression.get_type(), context);
                ...
                self.emit_binary_op(&left, &right, operator, result_type, context)
                    .map(Some)
            }
            ...
            Expression::ExponentiationExpression(expression) => {
                let target_type = self.resolve_slang_type(expression.get_type(), context);
                let left = expression.left_operand();
                let right = expression.right_operand();
                self.emit_binary_op(
                    &left,
            ...
            Expression::EqualityExpression(expression) => {
                ...
                self.emit_comparison(&left, &right, predicate, context)
                    .map(Some)
            }
            ...
            Expression::AndExpression(expression) => {
                let left = expression.left_operand();
                let right = expression.right_operand();
                self.emit_and(&left, &right, context).map(Some)
            }
            Expression::OrExpression(expression) => {
                let left = expression.left_operand();
                let right = expression.right_operand();
                self.emit_or(&left, &right, context).map(Some)
            }
            Expression::PostfixExpression(expression) => {
                ...
                self.emit_postfix(&operand, operator, context).map(Some)
            }
            Expression::PrefixExpression(expression) => {
                ...
                let operand = expression.operand();
                self.emit_prefix(operator, &operand, result_type, context)
                    .map(Some)
            }
            Expression::BitwiseAndExpression(expression) => {
                ...
                let left = expression.left_operand();
                let right = expression.right_operand();
                self.emit_binary_op(&left, &right, Operator::BitwiseAnd, result_type, context)
                    .map(Some)
            }
            Expression::BitwiseOrExpression(expression) => {
                ...
                let left = expression.left_operand();
                let right = expression.right_operand();
                self.emit_binary_op(&left, &right, Operator::BitwiseOr, result_type, context)
                    .map(Some)
            }
            Expression::BitwiseXorExpression(expression) => {
                ...
                let left = expression.left_operand();
                let right = expression.right_operand();
                self.emit_binary_op(&left, &right, Operator::BitwiseXor, result_type, context)
                    .map(Some)
            }
            ...
            Expression::FunctionCallExpression(call) => {
                self::call::CallEmitter::new(self).emit_function_call(call, context)
            }
            Expression::TupleExpression(tuple) => {
                ...
                self.emit(&inner, context)
            }
            Expression::ConditionalExpression(conditional) => {
                let result_type = self
                    .resolve_slang_type(conditional.get_type(), context)
                    .unwrap_or_else(|| {
                        Type::unsigned(context.melior, solx_utils::BIT_LENGTH_FIELD)
                    });
                let condition_value = self.emit_value(&conditional.operand(), context)?;
                let condition_boolean = self.emit_is_nonzero(condition_value, context);

                let result_slot = Place::stack(result_type, context);
                let parent = context.current_block();
                let (then_block, else_block) = parent.branch(condition_boolean, true, context);
                let else_block = else_block.expect("conditional expression emits an else arm");

                context.current_block = Some(then_block);
                let then_value = self.emit_value(&conditional.true_expression(), context)?;
                let then_cast = TypeConversion::from_target_type(result_type, context)
                    .emit(then_value, context);
                result_slot.store(then_cast, context);
                let then_end = context.current_block();
                then_end.r#yield(&[], context);

                context.current_block = Some(else_block);
                let else_value = self.emit_value(&conditional.false_expression(), context)?;
                let else_cast = TypeConversion::from_target_type(result_type, context)
                    .emit(else_value, context);
                result_slot.store(else_cast, context);
                let else_end = context.current_block();
                else_end.r#yield(&[], context);

                context.current_block = Some(parent);
                let result = result_slot.load(result_type, context);
                Ok(Some(result))
            }
            Expression::ArrayExpression(array_expression) => {
                ...
                let array_type =
                    TypeConversion::resolve_slang_type(&result_slang_type, None, context);
                let element_type =
                    TypeConversion::resolve_slang_type(&element_slang_type, None, context);
                let mut element_values = Vec::new();
                for item in array_expression.items().iter() {
                    let value = self.emit_value(&item, context)?;
                    let cast_value = TypeConversion::from_target_type(element_type, context)
                        .emit(value, context);
                    element_values.push(cast_value);
                }
                let value = Value::array_literal(&element_values, array_type, context);
                Ok(Some(value))
            }
            Expression::MemberAccessExpression(access) => {
                if let Some(value) = self.emit_struct_field(access, context)? {
                    Ok(Some(value))
                } else {
                    self::call::CallEmitter::new(self)
                        .emit_member_access(access, context)
                        .map(Some)
                }
            }
            Expression::IndexAccessExpression(index_access) => {
                self.emit_index_access(index_access, context)
            }
            _ => anyhow::bail!(
