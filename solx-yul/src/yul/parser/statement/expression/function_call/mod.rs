...
impl FunctionCall {
    ...
    pub fn into_llvm<'ctx>(
        ...
        if let Some((solidity_data, solc_location)) =
            context.solidity_mut().zip(self.solc_location.clone())
        {
        ...
        match name {
            ...
            Name::ReturnDataSize => solx_codegen_evm::return_data::size(context).map(Some),
            Name::ReturnDataCopy => {
                let arguments = self.pop_arguments_llvm::<3>(context)?;
                solx_codegen_evm::return_data::copy(
                    context,
                    arguments[0].into_int_value(),
                    arguments[1].into_int_value(),
                    arguments[2].into_int_value(),
